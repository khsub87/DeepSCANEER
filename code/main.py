...
def main(test_enzyme,enzyme_specific_prediction):
    ...
    # Generate features
    if os.path.isfile('../data/%s/input_feature.tsv'%test_enzyme):
        print(f"[{time.ctime()}] [Loading caculated input feature]")
        input_feature=pd.read_csv('../data/%s/input_feature.tsv'%test_enzyme,sep='\t')
    else:
        ...
        input_feature = calc_transformer_feature(test_enzyme, data_dir, ent_dic, attention, attention_trans)
        
    
    # Prepare test input
    x_test = [input_feature[mut].tolist() for mut in all_mutation]
    ...
    if enzyme_specific_prediction:
        ...
        enzyme_specific_data_num=len(mutagenesis_list)
        
        x_ft = [input_feature[mut].tolist() for mut in overlapped_mutation]
        ...
        result = predict_with_enzyme_specific_prediction(
            test_enzyme, pre_train_path,x_ft, y_ft, x_test
        )
    else:
    ...
    test_enzyme_df = pd.DataFrame(
        zip(all_mutation, *[result[i] for i in range(10)], result['prediction_score']),
...
pre_train_path = '../weight/pre_train_weight'

query_mutagenesis_path = f'{data_dir}/{test_enzyme}_score_ft.txt' if enzyme_specific_prediction else None

# Run
main(test_enzyme,enzyme_specific_prediction)